...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setMessage(data.message || 'Se ha enviado un enlace de recuperación si el correo existe.');
        setEmail('');
      } else {
    ...
    <div className="flex min-h-screen items-center justify-center bg-background px-4 relative overflow-hidden selection:bg-brand/20 selection:text-brand">
      ...
      <div className="w-full max-w-md p-8 sm:p-10 bg-surface/80 backdrop-blur-2xl rounded-[2rem] border border-border shadow-2xl shadow-brand/5 relative z-10 animate-in fade-in slide-in-from-bottom-8 duration-700">
        <div className="text-center mb-8">
          ...
          <h2 className="text-3xl font-black text-foreground tracking-tight mb-2">Recuperar Contraseña</h2>
          <p className="text-sm text-muted font-medium">Ingresa tu correo para recibir un enlace de recuperación</p>
        </div>
        ...
        <p className="mt-8 text-center text-sm font-medium text-muted">
          ¿Recuerdas tu contraseña? <Link href="/login" className="text-brand hover:underline font-bold">Volver al login</Link>
        </p>
      </div>
    </div>
  );
